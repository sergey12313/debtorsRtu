...
class RtuXmlGenerator {
    ...
    endEdit() {
        return new Promise((resolve, reject)=> {
            try {
                const xml = builder.create(this.commandsEdit,{encoding: 'utf-8'}).end(XML_BUILDER_CONFIG);
                resolve(xml);
            } catch (err) {
                reject(err)
            }
...
console.log(new RtuXmlGenerator(16666).setUserTerminal({ generatePassword: false, generatePassword: true }).setWebPassword().endEdit())